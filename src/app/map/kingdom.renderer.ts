...
import { Kingdom, Hex } from '../model/index';
...
export class KingdomRenderer {
  ...
  paintCountryside(hex: Hex) {
    let nbOfElement = (hex.resource ? 1 : 0) + (hex.guardTower ? 1 : 0) + (hex.pointOfInterest ? 1 : 0);
    if (nbOfElement > 0) {
      let buildingPlots = this.getBuildingPlots(hex, nbOfElement);
      let index = 0;
      if (hex.resource) {
  ...
  computePoint(hex: Hex, xOffset: number, yOffset: number) {
    let even = hex.y % 2 === 0;
    let offset = even ? EVEN_X_OFFSET : ODD_X_OFFSET;
