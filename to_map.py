...
for zoom in range(20,17,-1):
  ...
  call(["cmd", "/c","convert","stolen-lands-empty.png","-resize","{}%".format(reduction) ,zoomedImage])
  call(["cmd", "/c","convert","-crop","256x256",zoomedImage ,"tiles_%d.png"])
  os.remove(zoomedImage);
  total_tiles= len([name for name in os.listdir('.') if os.path.isfile(name) and name.startswith('tiles')])
  row = 0
  column = 0
